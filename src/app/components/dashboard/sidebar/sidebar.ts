...
export class Sidebar implements OnInit, OnDestroy {
  ...
  // Sous-menus filtrés par permissions
  gestionSubItems    = signal<any[]>([]);
  operationsSubItems = signal<any[]>([]);
  ...
  ngOnInit(): void {
    ...
    // Configurer les éléments de navigation selon le rôle + permissions
    this.setupNavigation();

    // Toujours rafraîchir le profil pour avoir les permissions à jour
  ...
  private setupNavigation(): void {
    ...
    const can = (perm: string) => this.userService.hasPermission(perm);
    ...
    const allSections = [
      {
        ...
        path: '/',
        visible: true  // tout le monde voit le dashboard
      },
      {
        id: 'users',
        title: 'Utilisateurs',
        ...
        path: '/users',
        visible: can('users.view') || ['superadmin', 'admin'].includes(currentRole)
      },
      {
        id: 'gestion',
        title: 'Gestion',
        ...
        hasChildren: true,
        visible: can('products.view') || can('stock.view') || ['admin', 'pharmacist', 'manager'].includes(currentRole)
      },
      {
        id: 'operations',
        title: 'Opérations',
        ...
        hasChildren: true,
        visible: can('sales.view') || can('customers.view') || ['cashier', 'admin', 'pharmacist', 'manager'].includes(currentRole)
      },
    ];

    // ── Sous-menus avec filtrage par permission ────────────────────────────
    const gestionItems = [
      {
        path: '/produits',
        ...
        visible: can('products.view') || ['admin', 'pharmacist', 'manager'].includes(currentRole)
      },
      {
        path: '/stocks',
        icon: 'M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4',
        label: 'Stocks',
        description: 'Mouvements et alertes',
        visible: can('stock.view') || ['admin', 'pharmacist', 'manager'].includes(currentRole)
      },
    ].filter(i => i.visible);
    ...
    const operationsItems = [
      {
        ...
        visible: can('sales.view') || ['cashier', 'admin', 'pharmacist', 'manager'].includes(currentRole)
      },
      {
        path: '/customers',
        ...
        visible: can('customers.view') || ['admin', 'pharmacist', 'cashier', 'manager'].includes(currentRole)
      },
    ].filter(i => i.visible);

    // Stocker les sous-menus filtrés pour le template
    this.gestionSubItems.set(gestionItems);
    this.operationsSubItems.set(operationsItems);
    ...
    this.navItems.set([...gestionItems, ...operationsItems]);
  }
